function dx = dynamics(t,x,t_end_of_previous_step,foot_index,Alpha,current_stance_foot_position)
    global t_global step_no torque
...
% VERIFIED    
    ...
    %% Platform motion
    v_tm = 0.05; step_length =0.1;
    th_0=8*pi/180; w=1*pi;
    th =  th_0*sin(w*t_global(end));  
    dth_dt = th_0*w*cos(w*t_global(end));
    ddth_dtt = -th_0*w*w*sin(w*t_global(end));
...
% current_stance_foot_position = SF_dynamic;
    ...
    if foot_index == 1 % FL foot is swing foot
        ...
        jj_c = jj_1_func(q,dq);
            ...
            l1=0.2092-v_tm*t_global(end) +0.1*step_no; %FL
            l2= -0.2295-v_tm*t_global(end) +0.1*step_no;%RR
            l3= 0.2506-v_tm*t_global(end) +0.1*step_no;%FR
            l4= -0.1881-v_tm*t_global(end) +0.1*step_no; %RL     
    ...
    elseif foot_index == 2 % RR foot is swing foot
        ...
        jj_c = jj_2_func(q,dq);
            ...
            l1= 0.2092-v_tm*t_global(end)+ step_length +0.1*step_no; %FL
            l2= -0.2295-v_tm*t_global(end) +0.1*step_no;%RR
            l3= 0.2506-v_tm*t_global(end)+0.1*step_no;%FR
            l4= -0.1881-v_tm*t_global(end)+0.1*step_no; %RL  
    ...
    elseif foot_index == 3 % FR foot is swing foot
        ...
        jj_c = jj_3_func(q,dq);  
            ...
            l1= 0.2092-v_tm*t_global(end) + step_length +0.1*step_no; %FL
            l2= -0.2295-v_tm*t_global(end) + step_length +0.1*step_no;%RR
            l3= 0.2506-v_tm*t_global(end) +0.1*step_no;%FR
            l4= -0.1881-v_tm*t_global(end) +0.1*step_no; %RL        
    ...
    elseif foot_index == 4 % RR foot is swing foot
        ...
        jj_c = jj_4_func(q,dq);  
            ...
            l1= 0.2092-v_tm*t_global(end) + step_length +0.1*step_no; %FL
            l2= -0.2295-v_tm*t_global(end) + step_length +0.1*step_no;%RR
            l3= 0.2506- v_tm*t_global(end) + step_length +0.1*step_no;%FR
            l4= -0.1881-v_tm*t_global(end)+0.1*step_no; %RL        
...
%%
    ...
    %B_overall = B; 
    [u,ss,uc,uv,ucm] = feedback_linearization(t,x,D,c_overall,B_overall,...
