...
%         zlabel('z')
        ...
        xc=[-0.5 1.4 1.4 -0.5]+0.1*t(i);
        yc=[-0.4 -0.4 0.4 0.4];
        zc=1.9*sin(th_0 + th_0*sin(w*t(i)-pi/2))*[0, 1, 1, 0];
        cc=[0.5 0.5 0.5 0.5];
...
%        delete(h)
    ...
    if want_record == 1
        video = VideoWriter('L_P_Treadmill_Pitching.avi');
        open(video)
        writeVideo(video,F)
        close(video)
    end
