...
%% Platform motion
 ...
 %%
    ...
    for i = 1:30:num_of_frame
       ...
       %subplot(5,1,1:3)
        %view(30,45)
        axis([-0.45 0.45 -0.4 0.4 -0.12 0.48])
        xlabel('x')
        ylabel('y')
        zlabel('z')
        xc=[-0.45 0.45 0.45 -0.45];
        yc=[-0.4 -0.4 0.4 0.4];
        zc=xc.*sin(th);
        cc=[0.5 0.5 0.5 0.5];
        plane = patch(xc,yc,zc,cc);
        ...
        RRlower2RFoot = line([RRlower(1),RRFoot(1)],[RRlower(2),RRFoot(2)],[RRlower(3),RRFoot(3)],'Color','red','LineWidth',3);
         ...
         t_unit =' s';
         t_lable = strcat(time,time_str,t_unit);
         % h = text(0.2,0,0.6,t_lable,'Color','red','FontSize',14);%text(2,8,'A Simple Plot','Color','red','FontSize',14)


        subplot(5,1,1:5);
        view(0,0)
        axis([-0.45 0.45 -0.3 0.3 -0.12 0.58])
        xlabel('x')
        ylabel('y')
        zlabel('z')
        ...
        zc=xc.*sin(th);
        cc=[0.6 0.6 0.6 0.6];
        plane2 = patch(xc,yc,zc,cc);
        time='t = ';
        time_str=num2str(t(i));
        t_unit =' s';
        t_lable = strcat(time,time_str,t_unit);
        h2 = text(0.2,0,0.5,t_lable,'Color','red','FontSize',14);%text(2,8,'A Simple Plot','Color','red','FontSize',14)
    ...
    if want_record == 1
        video = VideoWriter('StaticPC_dp.avi');
        video.Quality = 100;
        open(video)
        writeVideo(video,F)
        close(video)
