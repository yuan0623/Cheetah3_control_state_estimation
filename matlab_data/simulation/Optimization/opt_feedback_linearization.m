function u=opt_feedback_linearization(s,q_sol,dq_sol,D,c_overall,B_overall,Alpha,foot_index)
    q = q_sol;
    dq = dq_sol;
    global step_no
%%
        ...
        if foot_index == 1
            hc = hc_1_func(q)-[zeros(3,1); pi/2; 0; pi/2; 0 ; zeros(2,1)];  % 12x1
            hc_j = j_hc_1_func(q);
        ...
        elseif foot_index == 2
            %SF=SF_2_func(q);
            hc = hc_2_func(q)-[zeros(3,1); pi/2; 0; pi/2; 0 ; zeros(2,1)];  % 12x1
            hc_j = j_hc_2_func(q);
            hc_jj = jj_hc_2_func(q,dq);
            Alpha1=Alpha(2,:);
            %theta=q(1)-current_stance_foot_position(7)-0.3233 -0.025;%-2*0.1*(step_no);
            theta=q(1)-0.025-0.1*(step_no);  %(Alpha1(42)-Alpha1(36))/4;
            %theta=SF(1)-current_stance_foot_position(1);
            theta;
    
            %Alpha1(36:42)= Alpha1(36:42)+0.1*step_no;
        elseif foot_index == 3
            hc = hc_3_func(q)-[zeros(3,1); pi/2; 0; pi/2; 0; zeros(2,1)];  % 12x1
            hc_j = j_hc_3_func(q);
    ...
    theta_minus = 0.025; %base at end of step
