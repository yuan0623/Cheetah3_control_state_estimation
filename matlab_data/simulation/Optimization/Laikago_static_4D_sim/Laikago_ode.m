...
%clear all
tic
global s_overall y_global dy_global t_global global_position_reference Fr COP s_global dphi_global phi_global hc_global step_no
tspan=linspace(0,2,2000);
x0=zeros(1,36);
%q0=[0, 0, 0.27, 1.5708, 0, 1.5708, 0.04, 0.2, -1.26, -0.04, 0.2, -1.26, 0.04,0.2, -1.26, -0.04, 0.2, -1.26];
%q0=[0, 0, 0.35, 1.5708, 0, 1.5708, 0.04, 0, -0.6, -0.04, 0, -0.6, 0.04,0, -0.6, -0.04, 0,-0.6];
%q0=[0, 0.0143, 0.36, 1.5708, 0, 1.5708, 0.04, -0.125, -0.6, -0.04, -0.125, -0.6, 0.04,-0.125, -0.6, -0.04, -0.125,-0.6];
%q0=[0, 0, 0.4268, 0, 0, 0, -0.05, 0.55,-1.19306,0.05, 0.65,-1.2,-0.05, 0.65,-1.2, 0.05, 0.55,-1.19306];% On the go pose modified

q0 = [0, 0, 0.4251, 0, 0, 0, -0.05, 0.54,-1.2017,0.05, 0.72,-1.2,-0.05, 0.72,-1.2, 0.05, 0.54,-1.2017];% On the go pose modified;SL=0.16 
x0(1:18)=q0;
...
% alpha1 = -0.1*[0, 0.005, 0.01, 0.015, 0.02, 0.025, 0.0263];  %y base
% 
% alpha2 = [0.36, 0.362, 0.364, 0.365, 0.364, 0.362, 0.36];
...
% 
% alpha6 =  [0.2189,0.23,0.25,0.27,0.29, 0.305, 0.3189];
% alpha7 =  [0.115,0.115,0.115,0.115,0.115,0.115,0.115]; % y swing foot
...
%% 
load Alpha_initial_s16_h09_sp.mat;
alpha=initial_guess;
Alpha1 = alpha(1:56);
Alpha2 = alpha(57:112);
Alpha3 = alpha(113:168);
Alpha4 = alpha(169:224);
% %alpha(5)=alpha(5)+0.05;
% alpha(5)=alpha(5)+0.025;
% alpha(6)=alpha(6)+0.038;
% alpha(7)=alpha(7)+0.048;
% Alpha1=alpha;
% Alpha2=alpha;
% Alpha2(1:7)=alpha(7)+0.002;
% Alpha2(1:7) = Alpha2(1:7);%-1.2*[0,0.002,0.004,0.006,0.008,0.009,0.01];
...
% 
% Alpha3=alpha;
% Alpha3(1:7)=alpha(7)+0.002; %-0.013;
% Alpha3(1:7)=Alpha3(1:7)-1.5*[0,0.002,0.004,0.006,0.01,0.025,0.045];
% %Alpha3(43:49)=-(0.124/0.11)*alpha(43:49);
% Alpha3(43:49)=-alpha(43:49);
% Alpha4=alpha;
% Alpha4(1:7)=[alpha(7)-0.085, alpha(6)-0.07, alpha(5)-0.065,alpha(4)-0.055, alpha(3)-0.050, alpha(2)-0.035, alpha(1)];%[0, 0.005, 0.01, 0.015, 0.02, 0.025, 0.0263];
% 
% Alpha4(36:42)=alpha(36:42)-0.437*ones(1,7);

Alpha=[Alpha1;Alpha2;Alpha3;Alpha4];
opt_variable_4domain=[Alpha1,Alpha2,Alpha3,Alpha4];
