function [q_error,COP_,q_sol_whole_step,Fr,dq_sol_whole_step,phi_overall]...
    =opt_inv_solve_floating_base_eq(options,Alpha,foot_index)
   ...
   %q0=[0, 0, 0.27, 1.5708, 0, 1.5708, 0.04, 0.2, -1.26, -0.04, 0.2, -1.26, 0.04,0.2, -1.26, -0.04, 0.2, -1.26];
    ...
    q0=[0, 0, 0.3565, 1.5708, 0, 1.5708, 0.04, -0.23, -0.6, 0.038, -0.23, -0.6, 0.04,-0.23, -0.6, 0.04, -0.23,-0.6];
   ...
   % q_fval_overall=zeros(num_of_point,38);
    ...
    theta_plus = 0;
    theta_minus = (alpha(42)-alpha(36))/4;%0.025;%base motion after each leg movement
    ...
    for s=linspace(0,1,num_points)
        ...
        phi = [q1;ph;StanceFeet]-[zeros(2,1);0.3565; pi/2; 0; pi/2; 0 ; zeros(11,1)]; %Desired value
        phi_overall(i,:) = phi';
       ...
       % deviation(i,1) =dev;
        ...
        [COP1,Fr1] = opt_dynamics(s,q_sol,dq_sol,alpha,foot_index);
