function [u,s]=feedback_linearization(t,x,D,c_overall,B_overall,foot_index,Alpha,current_stance_foot_position,t_end_of_previous_step)
    ...
    global y_global dy_global  step_no offset t_global
...
%         %phi(12)=0.1;
    ...
    elseif want_bezier==1
        ...
        if foot_index == 1
            %SF=SF_1_func(q);
            hc = [hc_1_func(q)]-[zeros(2,1); 0*z_contact ; pi/2; 0; pi/2; zeros(2,1);0*z_contact];  % 9x1 -zp*sin(w*t_global(end))
            hc_j = j_hc_1_func(q);
        ...
        elseif foot_index == 2
            %SF=SF_2_func(q);
            hc = hc_2_func(q)-[zeros(2,1);0*z_contact; pi/2; 0; pi/2; zeros(2,1);0*z_contact];  % 9x1
            hc_j = j_hc_2_func(q);
            ...
            theta;
        elseif foot_index == 3
            hc = hc_3_func(q)-[zeros(2,1);0*z_contact; pi/2; 0; pi/2; zeros(2,1);0*z_contact];  % 9x1 -zp*sin(w*t_global(end))
            hc_j = j_hc_3_func(q);
        ...
        elseif foot_index == 4
            hc = hc_4_func(q)- [zeros(2,1);0*z_contact; pi/2; 0; pi/2; zeros(2,1); 0*z_contact];  % 9x1
            hc_j = j_hc_4_func(q);
...
%     theta_minus = (Alpha1(42)-Alpha1(36))/4; %0.025; %base at end of step
 %%%   theta = q1 + theta_plus -step_no*0.1013;
