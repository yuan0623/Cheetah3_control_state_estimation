...
%          h = text(0.54,0,0.5,t_lable,'Color','red','FontSize',14);%text(2,8,'A Simple Plot','Color','red','FontSize',14)
        subplot(5,1,1:3)
        view(30,45)
        axis([-0.35 1.75 -0.4 0.4 -0.12 0.48])
        xlabel('x')
        ylabel('y')
        zlabel('z')
        ...
        subplot(5,1,4:5);
        view(0,0)
        axis([-0.35 01.75 -0.3 0.3 -0.12 0.58])
        xlabel('x')
        ylabel('y')
        zlabel('z')
    ...
    if want_record == 1
        video = VideoWriter('Laikago_15_steps_2_views_with_plane.avi');
        video.Quality = 100;
        open(video)
        writeVideo(video,F)
        close(video)
