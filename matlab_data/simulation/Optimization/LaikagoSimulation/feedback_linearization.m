...
%         %phi(12)=0.1;
    ...
    elseif want_bezier==1
        ...
        if foot_index == 1
            SF=SF_1_func(q);
            hc = hc_1_func(q)-[zeros(3,1); current_stance_foot_position(1) ; zeros(5,1)];  % 12x1
            hc_j = j_hc_1_func(q);
            hc_jj = jj_hc_1_func(q,dq);
            
            theta=q(1)-current_stance_foot_position(1)+0.114;
            %theta=SF(1)-current_stance_foot_position(1);
            theta
            Alpha1=Alpha(1,:);
        elseif foot_index == 2
            %SF=SF_2_func(q);
            hc = hc_2_func(q)-[zeros(2,1); 0; current_stance_foot_position(1); zeros(5,1)];  % 12x1
            hc_j = j_hc_2_func(q);
            hc_jj = jj_hc_2_func(q,dq);
            
            theta=q(1)-current_stance_foot_position(7)-0.32;
            %theta=SF(1)-current_stance_foot_position(1);
            theta
            Alpha1=Alpha(2,:);
            
        elseif foot_index == 3
            hc = hc_3_func(q)-[zeros(2,1); 0; current_stance_foot_position(1);zeros(5,1)];  % 12x1
            hc_j = j_hc_3_func(q);
            hc_jj = jj_hc_3_func(q,dq);
            
            theta=q(1)-current_stance_foot_position(1)+0.21;
            theta
            Alpha1=Alpha(3,:);            
  
        elseif foot_index == 4
            hc = hc_4_func(q)-[zeros(2,1); 0; pi/2; 0; pi/2; current_stance_foot_position(1); zeros(2,1)];  % 12x1
            hc_j = j_hc_4_func(q);
            hc_jj = jj_hc_4_func(q,dq);

            theta=q(1)-current_stance_foot_position(7)-0.22;
