function [u,s]=feedback_linearization(t,x,D,c_overall,B_overall,foot_index,Alpha,current_stance_foot_position,t_end_of_previous_step)
    ...
    elseif want_bezier==1
        ...
        if foot_index == -1
            hc = hc_reduced_1_func(q)-[zeros(5,1);current_stance_foot_position(1);zeros(2,1);current_stance_foot_position(4);zeros(2,1)];
            hc_j = j_hc_reduced_1_func(q);
            hc_jj = jj_hc_reduced_1_func(q,dq);
            Alpha1=Alpha(1,:);
        elseif foot_index == 1
            hc = hc_reduced_2_func(q)-[zeros(5,1);current_stance_foot_position(1);zeros(2,1);current_stance_foot_position(4);zeros(2,1)];
            hc_j = j_hc_reduced_2_func(q);
            hc_jj = jj_hc_reduced_2_func(q,dq);
            Alpha1=Alpha(2,:);
        end
